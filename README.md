...
- TODO - WordPress plugin installation

## Authentication

- TODO: AWS authentication

## Configuration

WP Snapshots currently relies on AWS to store files and data. As such, you need to connect to a "repository" hosted on AWS. Each command supports a `--repository` argument to tell the script which AWS repository to use, but to bypass requiring that argument on every command, you can store the repository using the command below. After you have done so, WP Snapshots will use the first repository stored in your configuration by default.

* __wpsnapshots configure \<repository\> [--region] [--user_name] [--user_email]__
  ...
  ```
  wp wpsnapshots configure 10up
  ```
  __Example Usage Without Prompts (No Interaction) :__
  ```
  wp wpsnapshots configure yourcompany --user_name="Jane Smith" --user_email="noreply@yourcompany.com"
  ```

If WP Snapshots has not been set up for your team/company, you'll need to create the WP Snapshots repository:

```
wp wpsnapshots create-repository <repository>
```
